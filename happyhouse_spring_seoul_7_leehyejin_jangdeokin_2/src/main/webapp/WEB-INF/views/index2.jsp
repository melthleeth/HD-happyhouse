			...
			<div class="row">
				<div class="col-md-12">
					<div class="search-wrap-1 ftco-animate p-4 px-5" style="margin-top:0;">
						<form action="${root }/trade/list">
							<select name="searchField" id="searchField">
								<option value="LIST">전체</option>
								<option value="TITLE" <c:if test='${field eq "TITLE"}'> selected="selected"</c:if>>상품명</option>
								<option value="CATEGORY" <c:if test='${field eq "CATEGORY"}'> selected="selected"</c:if>>분류</option>
								<option value="PRICE" <c:if test='${field eq "PRICE"}'> selected="selected"</c:if>>가격</option>
		  				     </select> 
							<input type="text" id="searchText" name="searchText" value="${text}">  <!-- 문제 4: 검색조건 안날아가고 남아있도록 -->
